...
const UpdateUserModal: FC<IUpdateUserModalProps> = ({
  ...
  const [userProjectsIdInput, setUserProjectsIdInput] = useState<number | null>(
    null
  )
  ...
  useEffect(() => {
    if (userId) {
      UserInfoAPI.fetchUserById(token, userId)
        .then((res) => {
          console.log(res)
          if (res) {
  ...
  const onChangeUserRole = (e: React.FormEvent<HTMLFormElement>): void => {
    if (userId !== null) {
      UserInfoAPI.editUserRole(token, formRoleData, userId)
        .then(() => {
  ...
  const changeUserProjects = (): void => {
    if (userId !== null) {
      UserInfoAPI.editUserProject(token, formProjectsData)
        .then(() => {
          openNotification('The user data is saved!', 'topRight')
        })
        .catch((e) => openNotification(e, 'topRight'))
  ...
  const deleteUserProjects = (): void => {
    if (userId !== null) {
      UserInfoAPI.deleteUserProject(token, formProjectsData)
        .then(() => {
          openNotification('The user data is saved!', 'topRight')
        })
        .catch((e) => openNotification(e, 'topRight'))
  ...
  const changeUserTeam = (): void => {
    if (userId !== null) {
      UserInfoAPI.editUserTeam(token, formTeamData)
        .then(() => {
          openNotification('The user data is saved!', 'topRight')
        })
        .catch((e) => openNotification(e, 'topRight'))
  ...
  const deleteUserTeam = (): void => {
    if (userId !== null) {
      UserInfoAPI.deleteUserTeam(token, formTeamData)
        .then(() => {
          openNotification('The user data is saved!', 'topRight')
        })
        .catch((e) => openNotification(e, 'topRight'))
  ...
  const handleUserProjectsChange = (
    ...
    >
      <Form className='mb-8' onFinish={onChangeUserRole}>
        <Form.Item
          ...
          rules={[{ required: false, message: t('enter-user-role-title') }]}
        >
          <Input
            type='text'
            value={userRoleTitle ?? ''}
      ...
      <Form className='mb-8'>
        <Form.Item
          ...
          rules={[{ required: false, message: t('enter-user-project-id') }]}
        >
          <Input
            type='number'
            value={userProjectsIdInput ?? ''}
            onChange={handleUserProjectsChange}
          />
        </Form.Item>
        <div className='flex justify-center gap-12'>
          <Button
            className='flex justify-center items-center text-lg w-30 mt-5'
            type='primary'
            danger
            onClick={deleteUserProjects}
          >
            <h4 className='p-1'>{t('leave-project')}</h4>
          </Button>
          <Button
            className='flex justify-center items-center text-lg w-30 mt-5'
            onClick={changeUserProjects}
          >
      ...
      <Form>
        <Form.Item
          label={t('user-team-id')}
          rules={[{ required: false, message: t('enter-user-team-id') }]}
        >
          <Input
            type='number'
            value={userTeamId ?? ''}
